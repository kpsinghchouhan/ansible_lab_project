Kpsinghchouhan.Ansible_lab_project Run Role
========================
...
- name: Execute tasks on servers
  hosts: servers
  roles:
    - role: kpsinghchouhan.ansible_lab_project.run
      run_x: 42
```
...
```yaml
- name: Initialize the run role from kpsinghchouhan.ansible_lab_project
  hosts: servers
  gather_facts: false
  tasks:
    - name: Trigger invocation of run role
      ansible.builtin.include_role:
        name: kpsinghchouhan.ansible_lab_project.run
      vars:
        run_x: 42
```
